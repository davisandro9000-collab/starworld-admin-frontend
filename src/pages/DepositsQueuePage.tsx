// src/pages/DepositsQueuePage.tsx
...
export default function DepositsQueuePage() {
  ...
  // getPendingDeposits returns { deposits: AdminDeposit[]; total: number }
  ...
  const history = historyData?.deposits ?? []
  ...
  const openCredit = (dep: any) => {
    setSelected(dep)
    setCreditCoins(String(dep.amountUsd * COIN_RATE))
    setCreditOpen(true)
  }
  ...
  return (
    <div className="admin-page">
      <PageHeader
        ...
        subtitle="Review and credit or reject pending deposit submissions"
      />
            ...
            {!selected ? (
              ...
              >
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      <Badge variant={selected.user?.tier ?? 'bronze'} />
                      <span className="coin-chip text-xs">
                        <span className="coin-dot" />{(selected.user?.coinBalance ?? 0).toLocaleString()}
                      </span>
                    </div>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <Detail label="Amount" value={`$${selected.amountUsd} USD`} />
                  ...
                  <div className="flex flex-col gap-1">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {METHOD_LINKS[selected.method] && (
                        <a
                          href={METHOD_LINKS[selected.method](selected.txHash)}
                          target="_blank"
                          rel="noopener noreferrer"
                          className="text-cyan text-xs font-body hover:underline"
                        >
                          View ↗
                        </a>
                      )}
                ...
                <div className="card p-3 bg-gold/5 border-gold/20 border rounded-lg">
                  ...
                  <p className="font-heading font-bold text-white">
                    ${selected.amountUsd} × {COIN_RATE} ={' '}
                    <span className="text-gold">{selected.amountUsd * COIN_RATE} coins</span>
                  </p>
                </div>
                ...
                {selected.status === 'pending' && (
                  <div className="flex gap-3">
                    <Button variant="success" onClick={() => openCredit(selected)} className="flex-1">
                      Credit Deposit
                    </Button>
                    <Button variant="danger" onClick={() => openReject(selected)} className="flex-1">
                      Reject
                    </Button>
                  </div>
                )}
      ...
      <Modal open={creditOpen} onClose={() => setCreditOpen(false)} title="Credit Deposit">
        <div className="flex flex-col gap-4">
          <p className="font-body text-white/60 text-sm">
            ...
            <strong className="text-white">${selected?.amountUsd}</strong> {selected?.method} deposit.
          </p>
          <Input
            ...
            hint={`Default: $${selected?.amountUsd} × ${COIN_RATE} = ${(selected?.amountUsd ?? 0) * COIN_RATE} coins`}
          />
          <div className="bg-loss/10 border border-loss/20 rounded-lg p-3">
            <p className="font-body text-loss text-xs">
              ⚠ This action is irreversible. Coins will be credited immediately.
            </p>
          </div>
          <div className="flex gap-3 justify-end">
            <Button variant="ghost" onClick={() => setCreditOpen(false)}>Cancel</Button>
            <Button variant="success" onClick={handleCredit} loading={creditMutation.isPending}>
              Confirm Credit
            </Button>
          </div>
        </div>
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2">
          ...
          <Badge variant={dep.user?.tier ?? 'bronze'} />
        </div>
        <div className="flex items-center gap-2 mt-0.5">
          <span className="font-body text-white/40 text-xs">{dep.method}</span>
          <span className="font-body text-white/60 text-xs font-medium">${dep.amountUsd}</span>
        </div>
      </div>
