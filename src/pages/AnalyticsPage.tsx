...
export default function AnalyticsPage() {
  ...
  const totalUsers = (data.tierDistribution?.bronze ?? 0) + (data.tierDistribution?.silver ?? 0) + (data.tierDistribution?.platinum ?? 0);
            ...
            <tbody>
              {(data.gameStats ?? []).map((g: any) => (
                <tr key={g.gameType} className="border-b border-sw-border/30 last:border-0">
                  ...
                  <td className="py-2.5 font-body text-white/60 text-sm pr-4">{g.totalGames?.toLocaleString()}</td>
                  <td className="py-2.5 font-body text-cyan text-sm pr-4">{g.winRate}%</td>
                  <td className="py-2.5"><span className="coin-chip text-xs"><span className="coin-dot" />{g.coinsAwarded?.toLocaleString()}</span></td>
                </tr>
              ))}
            </tbody>
          </table>
